/**
 * Journey Analytics API
 * No description provided (generated by Openapi Generator https://github.com/openapitools/openapi-generator)
 *
 * The version of the OpenAPI document: 6.35.0
 * 
 *
 * NOTE: This class is auto generated by OpenAPI Generator (https://openapi-generator.tech).
 * https://openapi-generator.tech
 * Do not edit the class manually.
 *
 */

(function(root, factory) {
  if (typeof define === 'function' && define.amd) {
    // AMD.
    define(['expect.js', process.cwd()+'/src/index'], factory);
  } else if (typeof module === 'object' && module.exports) {
    // CommonJS-like environments that support module.exports, like Node.
    factory(require('expect.js'), require(process.cwd()+'/src/index'));
  } else {
    // Browser globals (root is window)
    factory(root.expect, root.JourneyAnalyticsApi);
  }
}(this, function(expect, JourneyAnalyticsApi) {
  'use strict';

  var instance;

  beforeEach(function() {
    instance = new JourneyAnalyticsApi.RecordColumn();
  });

  var getProperty = function(object, getter, property) {
    // Use getter method if present; otherwise, get the property directly.
    if (typeof object[getter] === 'function')
      return object[getter]();
    else
      return object[property];
  }

  var setProperty = function(object, setter, property, value) {
    // Use setter method if present; otherwise, set the property directly.
    if (typeof object[setter] === 'function')
      object[setter](value);
    else
      object[property] = value;
  }

  describe('RecordColumn', function() {
    it('should create an instance of RecordColumn', function() {
      // uncomment below and update the code to test RecordColumn
      //var instance = new JourneyAnalyticsApi.RecordColumn();
      //expect(instance).to.be.a(JourneyAnalyticsApi.RecordColumn);
    });

    it('should have the property handle (base name: "Handle")', function() {
      // uncomment below and update the code to test the property handle
      //var instance = new JourneyAnalyticsApi.RecordColumn();
      //expect(instance).to.be();
    });

    it('should have the property nameEx (base name: "NameEx")', function() {
      // uncomment below and update the code to test the property nameEx
      //var instance = new JourneyAnalyticsApi.RecordColumn();
      //expect(instance).to.be();
    });

    it('should have the property name (base name: "Name")', function() {
      // uncomment below and update the code to test the property name
      //var instance = new JourneyAnalyticsApi.RecordColumn();
      //expect(instance).to.be();
    });

    it('should have the property description (base name: "Description")', function() {
      // uncomment below and update the code to test the property description
      //var instance = new JourneyAnalyticsApi.RecordColumn();
      //expect(instance).to.be();
    });

    it('should have the property type (base name: "Type")', function() {
      // uncomment below and update the code to test the property type
      //var instance = new JourneyAnalyticsApi.RecordColumn();
      //expect(instance).to.be();
    });

    it('should have the property dataType (base name: "DataType")', function() {
      // uncomment below and update the code to test the property dataType
      //var instance = new JourneyAnalyticsApi.RecordColumn();
      //expect(instance).to.be();
    });

    it('should have the property isIndexed (base name: "IsIndexed")', function() {
      // uncomment below and update the code to test the property isIndexed
      //var instance = new JourneyAnalyticsApi.RecordColumn();
      //expect(instance).to.be();
    });

    it('should have the property isSortable (base name: "IsSortable")', function() {
      // uncomment below and update the code to test the property isSortable
      //var instance = new JourneyAnalyticsApi.RecordColumn();
      //expect(instance).to.be();
    });

    it('should have the property width (base name: "Width")', function() {
      // uncomment below and update the code to test the property width
      //var instance = new JourneyAnalyticsApi.RecordColumn();
      //expect(instance).to.be();
    });

    it('should have the property recordCount (base name: "RecordCount")', function() {
      // uncomment below and update the code to test the property recordCount
      //var instance = new JourneyAnalyticsApi.RecordColumn();
      //expect(instance).to.be();
    });

    it('should have the property precision (base name: "Precision")', function() {
      // uncomment below and update the code to test the property precision
      //var instance = new JourneyAnalyticsApi.RecordColumn();
      //expect(instance).to.be();
    });

    it('should have the property snapshot (base name: "Snapshot")', function() {
      // uncomment below and update the code to test the property snapshot
      //var instance = new JourneyAnalyticsApi.RecordColumn();
      //expect(instance).to.be();
    });

    it('should have the property deleteOnDataChange (base name: "DeleteOnDataChange")', function() {
      // uncomment below and update the code to test the property deleteOnDataChange
      //var instance = new JourneyAnalyticsApi.RecordColumn();
      //expect(instance).to.be();
    });

    it('should have the property numberofNulls (base name: "NumberofNulls")', function() {
      // uncomment below and update the code to test the property numberofNulls
      //var instance = new JourneyAnalyticsApi.RecordColumn();
      //expect(instance).to.be();
    });

  });

}));
